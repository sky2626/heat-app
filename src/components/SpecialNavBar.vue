<template>
  <header 
  class="bg-mycolor-100 sm:flex sm:justify-between sm:items-center sm:px-4 sm:py-3">
    <div 
    class="flex items-center justify-between px-4 py-3 sm:p-0">
      ...
      class="sm:hidden md:hidden ">
        <button 
        @click="isOpen = !isOpen" type="button" class="block text-gray-500 hover:texbluei4e focus:text-white focus:outline-none">
          <svg 
          class="h-6 w-6 fill-current" viewBox="0 0 24 24">
    ...
    <nav :class="isOpen ? 'block' : 'hidden'" class="px-2 pt-2 pb-4 md:flex sm:flex sm:p-0">
      <a href="#" class="mt-1 block px-8 py-3 text-xl text-white font-semibold rounded hover:bg-blue-400 md:mt-0 md:ml-2 sm:mt-0 sm:ml-2" >Home</a>
